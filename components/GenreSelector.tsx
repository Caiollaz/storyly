...
const GenreSelector: React.FC<GenreSelectorProps> = ({ onGenreSelect, isLoading, onLoadGame, saveExists }) => {
  ...
  return (
    <div className="w-full max-w-2xl mx-auto text-center p-8">
      <h1 className="text-4xl md:text-5xl font-bold text-[var(--text-primary)] mb-2 title-epic">{t('title')}</h1>
      <p className="text-lg text-[var(--text-secondary)] mb-8 body-text">{t('subtitle')}</p>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
       ...
       <div className="my-6">
        <button
            ...
            className="w-full p-4 bg-[var(--bg-secondary)] border border-[var(--border-color)] rounded-lg hover:bg-[var(--text-accent)] hover:border-[var(--text-accent)] focus:outline-none focus:ring-2 focus:ring-[var(--focus-ring)] transition-colors duration-200 disabled:opacity-50 disabled:cursor-not-allowed body-text"
        >
       ...
       <div className="relative my-6">
        ...
        <div className="relative flex justify-center">
            <span className="bg-[var(--bg-primary)] px-2 text-sm text-[var(--text-muted)] body-text">{t('or_divider')}</span>
        </div>
      </div>
